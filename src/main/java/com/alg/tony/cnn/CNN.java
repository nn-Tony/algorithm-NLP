...
public class CNN implements Serializable {
     ...
     * @return
     */
    public int predict_probility(Dataset.Record record, double[] out) {
//        Log.i("begin predict");
    ...
    public void predict(Dataset testset, String fileName) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if(lable == record.getLable()){
                	//System.out.println(lable +" "+ record.getLable());
                    rightcount+=1;
                }
                writer.write(lable + "\n");
            }
            
            System.out.println("N2N:  "+rightcount);
            System.out.println("all num:  "+count);
    ...
    private boolean setOutLayerErrors(Dataset.Record record) {
        ...
        Double aaa = record.getLable();
        int lable = record.getLable().intValue();
        //Log.i(record.getLable() + "outmaps:" +Util.fomart(outmaps)+ Arrays.toString(target));
        ...
        int index = Util.getMaxIndex(outmaps);
       /* int sum = 0;
        System.out.println(lable+"\t"+index+"\t");
        for(int i=0;i<outmaps.length;i++){
            System.out.print(outmaps[i]+" ");
            sum +=outmaps[i];
        }
        System.out.println("\n");*/
        if(index == lable){
        ...
        for (int i = 0; i < mapSize.x; i++) {
            for (int j = 0; j < mapSize.y; j++) {
                ...
                /*if(i>=0){
                    ...
                    System.out.println(mapSize.x/2 * i + j);
                    System.out.println(attr[mapSize.x/2 * i + j]);
                }*/
                //inputLayer.setMapValue(0, i, j, attr[mapSize.x * i + j]);
                inputLayer.setMapValue(0, i, j, attr[mapSize.y * i + j]);
                //inputLayer.setMapValue(0, i, j, attr[mapSize.x/2 * i + j]);
            }
        }
    }
    ...
    public void setup(int batchSize) {
        ...
        for (int i = 1; i < layers.size(); i++) {
            ...
            case samp:
                ...
                //System.out.println(frontLayer.getMapSize()+" "+layer.getScaleSize());
...
//    }
    ...
    public Map<Integer, String> predictType(Dataset testset) {
        ...
        while (iter.hasNext()) {
            ...
            int lable = Util.getMaxIndex(out);
            labelmap.put(lable, out[0]+"#"+out[1]+"#"+out[2]);
        }
